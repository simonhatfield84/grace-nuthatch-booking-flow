-- Create square_payments table to track payment lifecycle
...
-- Create square_order_line_items table for itemized bill data
...
CREATE POLICY "venue_staff_read_payments"
  ON public.square_payments
  FOR SELECT
  USING (true);

-- RLS policies for square_order_line_items
CREATE POLICY "service_role_all_line_items"
  ON public.square_order_line_items
  FOR ALL
  USING (true)
  WITH CHECK (true);

CREATE POLICY "venue_staff_read_line_items"
  ON public.square_order_line_items
  FOR SELECT
  USING (true);