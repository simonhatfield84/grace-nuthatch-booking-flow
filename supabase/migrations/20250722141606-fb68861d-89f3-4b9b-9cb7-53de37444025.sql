...
-- Recreate the missing expire_pending_payments function and payment_analytics table
...
CREATE POLICY IF NOT EXISTS "Venue users can view their payment analytics" 
  ON public.payment_analytics 
  FOR SELECT 
  USING (venue_id = get_user_venue(auth.uid()));

CREATE POLICY IF NOT EXISTS "System can create payment analytics" 
  ON public.payment_analytics 
  FOR INSERT 
  WITH CHECK (true);
...
SELECT cron.unschedule('expire-pending-payments') WHERE EXISTS (
...
-- Clean up existing stuck bookings in pending_payment status
UPDATE public.bookings 
SET 
  status = 'expired',
  cancellation_reason = 'payment_timeout_cleanup',
  payment_timeout_at = NOW(),
  updated_at = NOW()
WHERE status = 'pending_payment' 
  AND created_at < NOW() - INTERVAL '5 minutes';
