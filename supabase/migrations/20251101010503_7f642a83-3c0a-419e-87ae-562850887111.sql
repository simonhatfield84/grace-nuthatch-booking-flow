-- Enhanced update_guest_metrics to use guest_id from order_links and email/phone matching
...
BEGIN
  ...
  INTO 
    ...
    v_last_visit
  FROM bookings b
  LEFT JOIN order_links ol ON ol.visit_id = b.id::TEXT
  LEFT JOIN square_orders so ON ol.order_id = so.order_id AND so.state = 'COMPLETED'
  WHERE (
    ol.guest_id = p_guest_id  -- Direct guest_id link
    OR (v_guest_email IS NOT NULL AND LOWER(b.email) = LOWER(v_guest_email))  -- Email match
    OR (v_guest_phone IS NOT NULL AND b.phone = v_guest_phone)  -- Phone match
  )
