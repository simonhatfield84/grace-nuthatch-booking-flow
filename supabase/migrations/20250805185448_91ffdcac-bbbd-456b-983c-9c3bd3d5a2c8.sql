...
-- Update the create_default_email_templates function to include the missing payment-related templates
...
BEGIN
  ...
  VALUES 
    ...
    -- NEW: Payment request template (replaces hardcoded email)
    (p_venue_id, 'payment_request', 'venue',
     'Payment Required - ' || venue_record.name,
     '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="text-align: center; margin-bottom: 30px;">
          <img src="/lovable-uploads/0fac96e7-74c4-452d-841d-1d727bf769c7.png" alt="The Nuthatch" style="height: 60px; width: auto; margin: 20px 0;" />
          <p style="color: #64748b; margin: 5px 0;">Payment Required</p>
        </div>
        <div style="background: #f8fafc; padding: 30px; border-radius: 8px;">
          <h2 style="color: #1e293b; margin-top: 0;">Payment Required for Your Booking</h2>
          <p>Dear {{guest_name}},</p>
          <p>Your booking at {{venue_name}} requires payment to be confirmed.</p>
          <div style="background: white; padding: 20px; border-radius: 5px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #000000;">Booking Details</h3>
            <p><strong>Reference:</strong> {{booking_reference}}</p>
    ...
    -- NEW: 22-hour payment reminder template
    (p_venue_id, 'payment_reminder_22h', 'venue',
     'Payment Reminder - Only 2 Hours Left - ' || venue_record.name,
     '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="text-align: center; margin-bottom: 30px;">
          <img src="/lovable-uploads/0fac96e7-74c4-452d-841d-1d727bf769c7.png" alt="The Nuthatch" style="height: 60px; width: auto; margin: 20px 0;" />
          <p style="color: #f59e0b; margin: 5px 0;">⚠️ Payment Reminder</p>
        </div>
        <div style="background: #fffbeb; border: 2px solid #f59e0b; padding: 30px; border-radius: 8px;">
          <h2 style="color: #92400e; margin-top: 0;">⏰ Only 2 Hours Left to Complete Payment</h2>
          <p>Dear {{guest_name}},</p>
          <p>This is a friendly reminder that your booking payment is due soon!</p>
          <div style="background: white; padding: 20px; border-radius: 5px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #000000;">Booking Details</h3>
            <p><strong>Reference:</strong> {{booking_reference}}</p>
            <p><strong>Service:</strong> {{service}}</p>
     ...
     'Sent 22 hours after payment request as final reminder',
     true, false, basic_design),
     
    -- NEW: Payment expired template
    (p_venue_id, 'payment_expired_24h', 'venue',
     'Booking Expired - Payment Not Received - ' || venue_record.name,
     '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="text-align: center; margin-bottom: 30px;">
          <img src="/lovable-uploads/0fac96e7-74c4-452d-841d-1d727bf769c7.png" alt="The Nuthatch" style="height: 60px; width: auto; margin: 20px 0;" />
          <p style="color: #ef4444; margin: 5px 0;">❌ Booking Expired</p>
        </div>
        <div style="background: #fef2f2; border: 2px solid #ef4444; padding: 30px; border-radius: 8px;">
          <h2 style="color: #dc2626; margin-top: 0;">Booking Payment Window Expired</h2>
          <p>Dear {{guest_name}},</p>
          <p>Unfortunately, the payment window for your booking at {{venue_name}} has expired.</p>
          <div style="background: white; padding: 20px; border-radius: 5px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #000000;">Expired Booking Details</h3>
            <p><strong>Reference:</strong> {{booking_reference}}</p>
            <p><strong>Service:</strong> {{service}}</p>
            <p><strong>Date:</strong> {{booking_date}}</p>
            <p><strong>Time:</strong> {{booking_time}}</p>
          ...
          <p>We understand that sometimes plans change. If you would still like to visit us, please feel free to make a new booking or contact us directly.</p>
        </div>
        <div style="text-align: center; color: #94a3b8; font-size: 12px; margin-top: 30px;">
          <p>{{email_signature}}</p>
          <p style="margin-top: 20px; font-size: 10px; color: #999;">Powered by Grace</p>
     ...
     'Sent when payment expires after 24 hours',
     true, false, basic_design),
     
    -- Continue with other existing templates...
...
-- Create a new table to track payment requests with expiry times
