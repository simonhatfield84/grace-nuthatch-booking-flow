...
import { Plus, Trash2, Calendar as CalendarIcon, X, Edit3, Save, ChevronDown, ChevronUp } from "lucide-react";
...
interface InlineBookingWindowManagerProps {
  serviceId: string;
}
...
export function InlineBookingWindowManager({ serviceId }: InlineBookingWindowManagerProps) {
  ...
  const handleSubmit = () => {
    if (!serviceId || !userVenue) {
      toast({
        title: "Error",
        description: "Service ID or venue ID is missing.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (editingWindowId) {
      updateWindowMutation.mutate({ ...windowData, id: editingWindowId });
    } else {
  ...
  const toggleWindowExpansion = (windowId: string) => {
  ...
  if (!serviceId || !userVenue) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <p>Save the service first to configure booking availability windows.</p>
      </div>
    );
  }
            ...
            <div className="flex gap-2 pt-4 border-t">
              ...
              >
                {createWindowMutation.isPending || updateWindowMutation.isPending 
                  ? 'Saving...' 
                  : editingWindowId ? 'Update Window' : 'Add Window'
                }
              </Button>
              <Button 
                variant="outline" 
